...
const SingleArticleA1 = () => {
	...
	useEffect(() => {
		const fetchArticle = async () => {
			try {
				const response = await fetch(`${BASE_URL}/api/readings/${params.id}`, { credentials: 'include' })
		...
		<div className="bg-background">
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
				<div className="flex flex-row items-center justify-between gap-6">
					<h1 className="text-2xl sm:text-4xl lg:text-5xl font-bold text-foreground">{article.title}</h1>
			...
			{article.imageUrl && (
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
					<div className="relative w-full h-64 sm:h-80 md:h-96 lg:h-[28rem] rounded-lg overflow-hidden shadow-lg">
						<Image src={article.imageUrl} alt={article.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 1200px" priority />
					</div>
				</div>
			)}
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-16">
					<div className="border rounded-md overflow-hidden bg-background">
						...
						<div className="px-6 py-6">
							<div className="prose prose-lg max-w-none text-foreground whitespace-pre-wrap">{article.content}</div>
						</div>
					</div>
				</div>
			)}
									...
									{openTasks[idx] && (
										<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: ANIM_DURATION }} className="overflow-hidden border-t">
											<div className="px-4 py-3">
												...
												{article?.taskPdf && (
													<div className="mt-3">
														<PdfButton pdfUrl={article.taskPdf} onOpen={(url) => openPdf(url, `Task ${idx + 1} PDF`)} label={`Task ${idx + 1} PDF`} />
													</div>
												)}
											</div>
										</motion.div>
						...
						<div className="border rounded-md overflow-hidden">
							...
							<AnimatePresence initial={false}>
								{openTasks[0] && (
									<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: ANIM_DURATION }} className="overflow-hidden border-t">
										<div className="px-4 py-3">
											{article?.taskPdf && <PdfButton pdfUrl={article.taskPdf} onOpen={(url) => openPdf(url, 'Task PDF')} label="Task PDF" />}
										</div>
									</motion.div>
								)}
							</AnimatePresence>
