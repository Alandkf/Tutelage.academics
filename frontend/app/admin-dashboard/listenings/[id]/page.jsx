...
export default function AdminListeningDetailPage() {
  ...
  const handleEditSuccess = async (formData) => {
    try {
      ...
      setShowEdit(false)
      fetchListening()
      toast('Listening updated successfully', { variant: 'success' })
    } catch (e) {
      toast(e.message || 'Failed to update listening', { variant: 'destructive' })
    }
  }

  const confirmDelete = async () => {
    try {
      await fetch(`${BASE_URL}/api/audios/${params.id}`, { method: 'DELETE', credentials: 'include' })
      toast('Listening deleted successfully', { variant: 'destructive' })
      router.push('/admin-dashboard/listenings')
    } catch {
      toast('Failed to delete listening', { variant: 'destructive' })
    }
  }
