...
const EslAudioForm = ({ mode = 'create', initialValues = null, onSuccess, onCancel }) => {
	...
	useEffect(() => {
		if (mode === 'edit' && initialValues) {
			...
			// Parse tags properly from initialValues
			let parsedTags = [];
			if (Array.isArray(initialValues.tags)) {
				parsedTags = initialValues.tags.filter(Boolean);
			} else if (typeof initialValues.tags === 'string' && initialValues.tags) {
				parsedTags = initialValues.tags.split(',').map(t => t.trim()).filter(Boolean);
			}
		...
		<div className="max-h-[70vh] overflow-y-auto pr-2">
			<form onSubmit={handleSubmit} className="space-y-4">
				...
				<div>
					...
					<Input id="audioRef" value={formData.audioRef} onChange={(e) => setFormData(v => ({ ...v, audioRef: e.target.value }))} placeholder="https://..." required />
				</div>
				...
				<div>
					...
					<Textarea id="transcript" value={formData.transcript} onChange={(e) => setFormData(v => ({ ...v, transcript: e.target.value }))} rows={4} />
				</div>
