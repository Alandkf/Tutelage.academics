  ...
  const fetchArticles = async (page) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/api/readings/paginated?page=${page}&limit=${itemsPerPage}&level=${encodeURIComponent(levelParam)}`,
        { credentials: 'include' }
      )
      const data = await response.json()
      if (data?.success && data?.data) {
        // backend returns data.data.readings for paginated readings
        setArticles(Array.isArray(data.data.readings) ? data.data.readings : [])
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
          {loading ? (
            ...
            articles?.map((article) => (
              <Link
                key={article.id}
                href={`/skills/reading/b1/${article.id}`}
                className="group"
              >
...
export default ReadingArticleGridB1
